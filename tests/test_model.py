...
def test_depth_factor_anchor_cap_and_direction():
    """1 at the 30-degree anchor, >1 steeper, <1 gentler, capped at 60."""
    from h_sim.model import physical as P

    t30, t45, t60 = np.tan(np.radians([30.0, 45.0, 60.0]))
    assert P.depth_factor(np.array([t30]), 2.0)[0] == 1.0
    assert P.depth_factor(np.array([t45]), 2.0)[0] > 1.0
    assert P.depth_factor(np.array([0.1]), 2.0)[0] < 1.0
    # beyond 60 degrees the law stops extrapolating
    f60 = P.depth_factor(np.array([t60]), 2.0)[0]
    f80 = P.depth_factor(np.array([np.tan(np.radians(80.0))]), 2.0)[0]
    assert f60 == f80
    # off costs nothing and changes nothing
    assert P.depth_factor(np.array([t45]), 0.0) == 1.0


def test_depth_term_leaves_the_anchor_slope_untouched():
    """At exactly 30 degrees, FS with and without depth_k agree."""
    from h_sim.model import physical as P

    slope = np.full(5, np.tan(np.radians(30.0)))
    sca = np.linspace(50.0, 500.0, 5)
    a = P.factor_of_safety(slope, sca, 0.1, 35.0, 0.002)
    b = P.factor_of_safety(slope, sca, 0.1, 35.0, 0.002, depth_k=2.5)
    assert np.allclose(a, b)


def test_depth_term_moves_cohesion_and_wetness_together():
    """On steep dry ground FS rises (more relative cohesion); the same
    factor also raises R/T, so wetness rises where drainage converges."""
    from h_sim.model import physical as P

    steep = np.array([np.tan(np.radians(45.0))])
    # dry, cohesive: the cohesion side wins
    fs0 = P.factor_of_safety(steep, np.array([1.0]), 0.2, 35.0, 1e-6)
    fs2 = P.factor_of_safety(steep, np.array([1.0]), 0.2, 35.0, 1e-6,
                             depth_k=2.0)
    assert fs2[0] > fs0[0]
    # wetness carries the same 1/h: rt is scaled by the factor
    f = P.depth_factor(steep, 2.0)
    w0 = P.wetness(np.array([300.0]), steep, 0.002)
    w2 = P.wetness(np.array([300.0]), steep, 0.002 * f)
    assert w2[0] > w0[0]


def test_soil_parameters_round_trip_and_legacy_load():
    from h_sim.model import physical as P

    p = P.SoilParameters((0.0, 0.2), (30.0, 40.0), (1e-4, 1e-2), depth_k=1.0)
    assert P.SoilParameters.from_dict(p.as_dict()) == p
    # a fit written before the depth term existed loads as uniform depth
    legacy = {"cohesion": [0.0, 0.2], "friction_deg": [30.0, 40.0],
              "rt": [1e-4, 1e-2]}
    assert P.SoilParameters.from_dict(legacy).depth_k == 0.0


def test_fit_recovers_a_depth_signal_planted_in_synthetic_data():
    """Labels generated by a depth_k = 2.5 model: the augmented grid should
    beat the uniform grid in-sample and choose a nonzero depth_k."""
    from h_sim.model import physical as P

    rng = np.random.default_rng(3)
    n = 800
    slope = rng.uniform(np.tan(np.radians(15.0)), np.tan(np.radians(50.0)), n)
    sca = rng.uniform(30.0, 2000.0, n)
    truth = P.SoilParameters((0.0, 0.15), (30.0, 40.0), (4e-5, 2e-3),
                             depth_k=2.5)
    p_true = P.failure_probability(slope, sca, truth, n_samples=80, seed=1)
    y = (rng.uniform(size=n) < p_true).astype(float)
    if y.sum() < 30 or (1 - y).sum() < 30:
        raise AssertionError("degenerate synthetic labels")

    pres, bg = y == 1, y == 0
    base = P.fit_parameters(slope[pres], sca[pres], slope[bg], sca[bg],
                            n_samples=40, seed=0)
    aug = P.fit_parameters(slope[pres], sca[pres], slope[bg], sca[bg],
                           n_samples=40, seed=0,
                           grid=P.parameter_grid((0.0,)
                                                 + P.DEPTH_K_CANDIDATES))
    assert aug["parameters"].depth_k > 0.0
